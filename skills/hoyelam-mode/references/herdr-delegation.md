...
Apply this adapter when the user requests Herdr orchestration or hoyelam-mode detects a managed Herdr environment for useful, authorized delegation. Explicit user preferences override automatic selection. Running inside Herdr alone does not require delegation. Hoyelam-mode still selects the workflow and verification depth; Herdr supplies worker terminals and their control surface.

## Route and bootstrap

1. Check `HERDR_ENV=1` and CLI availability with `command -v herdr` before inspecting or controlling the session. If either is absent, Herdr is unavailable. For automatic selection, return to hoyelam-mode’s native/direct fallback; for an explicit Herdr requirement, report the blocker and continue independent direct work without silently substituting another route.
2. Read the Herdr skill if it is available. Otherwise load the release-matched instructions with `herdr --skill`. This adapter supplies the automatic-selection policy; use the bundled skill for CLI operation and session safety even if its standalone trigger requires an explicit Herdr mention. Use `herdr --help` and the relevant command groups to confirm the installed syntax; do not run bare `herdr` inside a pane. Before creating panes or starting workers, use the documented read-only session/current-pane query to verify that the managed session is reachable and the caller can be resolved. Use a bounded timeout. An environment marker or installed binary alone is insufficient. Distinguish sandbox-denied socket access from an unreachable session: follow the harness’s permission and retry rules before concluding that Herdr is unavailable. If the check still fails or access is unavailable, apply the same automatic-fallback versus explicit-blocker rule from step 1. Do not launch or repair Herdr merely to satisfy detection.
3. Keep the current agent as the lead. Choose workers from independently ready work and the actual configured capacity across Herdr and native delegation combined.
