...
4. Use available authorized delegation tools. Follow [hoyelam-mode’s execution routing](../hoyelam-mode/SKILL.md#single-task-delegation): honor explicit preferences, automatically select the [Herdr adapter](../hoyelam-mode/references/herdr-delegation.md) for a reachable managed Herdr session, and otherwise use available native delegation (the [Codex adapter](../hoyelam-mode/references/codex-delegation.md) in Codex). An explicit Herdr requirement must not silently fall back. No particular hosting service, model, PR stack tool, or CI service is required.
